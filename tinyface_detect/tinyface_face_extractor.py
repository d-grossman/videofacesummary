...
def main(
    ...
    print(
        "Processing image {0} with Tiny Face Probability threshold: {1}, NMS threshold {2}".format(
            image_filename,
            prob_thresh,
            nms_thresh))
    sys.stdout.flush()
    ...
    print(
        "{0} Tiny Face bounding boxes written to: {1}".format(
            len(tinyfaces),
            output_filename))
    sys.stdout.flush()
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Extract faces from image with Tiny Face')
    # Required args
    parser.add_argument(
        "image_filename",
        type=str,
