...
# Process extracted face chips through pretrained Torch model to obtain
# vector represenations
def vectorize_chips(chips, chip_size, net, aligner):
    ...
    faces = list()
    for chip in chips:
        alignedface = aligner.align(
            chip_size, chip, landmarkIndices=AlignDlib.OUTER_EYES_AND_NOSE)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Vectorize and group previously detected faces in video and images'
    ...
    if not os.path.isfile(args.facenet_model):
        print(
            "Error - Required Torch model was not found. Please verify your Torch model is located at {0}".format(
                args.facenet_model))
        quit()
    elif not os.path.isfile(args.dlibFacePredictor):
        print(
            "Error - Required dlib landmark model was not found. Please verify your landmark model is located at {0}".format(
