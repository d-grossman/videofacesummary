...
def process_image(
    ...
    face_locations, num_detections = identify_chips(
        image, frame_number, margin, min_size, threshold, scale_factor, reduceby, pnet, rnet, onet)
...
def process_video(
    ...
    while keep_going:
        ...
        face_locations, detections = identify_chips(
            image, frame_number, margin, min_size, threshold, scale_factor, reduceby, pnet, rnet, onet)
        if detections > 0:
            # print((frame_number,face_locations))
            # sys.stdout.flush()
...
def identify_chips(
    ...
    _, list_face_locations = align.load_and_align_data(
        resized_image, margin, min_size, threshold, scale_factor, pnet, rnet, onet)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Identify bounding boxes for faces in video and images using MTCNN')
