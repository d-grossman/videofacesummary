...
def main(image_foldername):

	# Thresholds to keep candidate face
	MAX_INPUT_DIM=5000.0
	prob_thresh = 0.5
	nms_thresh = 0.1	
	print("Probability threshold: %s, NMS threshold %s" % (prob_thresh,nms_thresh))
	sys.stdout.flush()

	clusters, averageImage = loadmeta('/tinyface/hr101_mxnet/hr_res101.mat')

	clusters_h = clusters[:,3] - clusters[:,1] + 1
	clusters_w = clusters[:,2] - clusters[:,0] + 1
	normal_idx = np.where(clusters[:,4] == 1)

	filtered_folder = [ file for file in os.listdir(image_foldername) if os.path.splitext(file)[1] in ['.png','.jpg','.jpeg','.gif'] ]
	processed_folder=os.path.join(image_foldername,"processed")
	if not os.path.exists(processed_folder): os.mkdir(processed_folder)
	
	for image_name in filtered_folder:
		raw_img = cv2.imread(os.path.join(image_foldername,image_name))
		...
		print "Processing time:", time.time()-start, "secs."
		...
		print("Total detected faces: %d" % len(refind_bboxes))
		output_file = os.path.join(processed_folder, os.path.splitext(image_name)[0]+"_"+str(len(refind_bboxes))+"faces"+os.path.splitext(image_name)[1])
		cv2.imwrite(output_file,cv2.cvtColor(raw_img, cv2.COLOR_RGB2BGR))
		print("Written to: %s" % output_file)
		sys.stdout.flush()

if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(description='Run Tiny Face Detector on Images')
    # Required args
    parser.add_argument("image_foldername",
                        type=str,
