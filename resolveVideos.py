...
def main(
    ...
    if os.path.isfile(in_hash_table):
        ...
        print("Loaded hash table from {0} with {1} entries".format(
            in_hash_table, len(hash_table['hash_to_file'])))
    else:
        print("No hash table pickle file found at {0}, starting with blank table".format(
            in_hash_table))
        hash_table = defaultdict(dict)

    filtered_folder = [file for file in os.listdir(
        detected_faces_folder) if file.endswith('face_detected.pickle')]
    ...
    for current_file in filtered_folder:
        ...
        # Establish a reference set of faces from first file if no reference
        # set exists
        if cold_start:
            for current_face_id in current_faces:
                ...
                if write_to_hash:
                    contenthash = current_faces[
                        current_face_id]['file_content_hash']
            ...
            reference_vectors_tuples = [
                (reference_faces[x]['face_vec'], x) for x in reference_faces]
            ...
            for current_face_id in current_faces:
                ...
                match = face.compare_faces(reference_vectors_only, current_faces[
                                           current_face_id]['face_vec'], tolerance)
                match_indexes = [i for i, x in enumerate(match) if x]

                # Merge matches into reference set
                # TODO - Should multiple reference face matches be averaged
                # with the current face as a new reference entity?
            ...
            # Pop off merged faces and add remaining new faces into reference
            # set
    ...
    print(
        "Reviewed {0} faces and merged {1} duplicate faces".format(
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Merge faces detected from video via directFeatures')
