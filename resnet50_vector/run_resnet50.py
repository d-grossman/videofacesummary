...
def extract_chips(filename, bounding_boxes, chip_size, file_content_hash, tolerance, jitters, verbose=False):
...
def vectorize_chips(chips, chip_size, jitters):
    ...
    for chip in chips:
        ...
        vectorized_faces += face.face_encodings(adjusted_face, [(0, chip_size, chip_size, 0)], jitters)
...
def normalize_faces(pic, chip_size):
    ...
    landmarks = get_face_landmarks(
        face.pose_predictor, pic, dlib.rectangle(0, 0, pic.shape[1], pic.shape[0]))
        ...
        if os.path.isfile(file_with_path) and file_digest(file_with_path) == file_content_hash:
            if len(bounding_boxes) > 0:
                ...
                if verbose and bounding_boxes[0][0] == -1:
                    ...
                    extract_chips(filename, bounding_boxes, chip_size, file_content_hash, tolerance, jitters)
