...
# point this to a directory of directories containing example cropped
# images of ppl of interest
...
# plot dots where faces are, black dots for video data, colored dots for
# people of interest
...
def tsne_html(datastore, d, spread=50, tsne=True):
    ...
    p = figure(plot_width=800, plot_height=800, x_range=(-80, 80),
               y_range=(-80, 80), output_backend="webgl")
...
def showResults(threshold, key, tasking, people):
    ...
    for idx, f in enumerate(sorted_by_distance):
        ...
        plt.title(
            "label:{0} dist:{1:02.3} det:{2:03}".format(
                f[0], f[2], f[1]))
        plt.axis('off')
