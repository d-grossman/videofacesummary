...
def main(image_foldername):
    ...
    nms_thresh = 0.1
    print(
        "Probability threshold: %s, NMS threshold %s" %
    ...
    for image_name in filtered_folder:
        ...
        min_scale = min(
            np.floor(
                np.log2(
                    np.max(
                        clusters_w[normal_idx] /
                        raw_w))),
            np.floor(
                np.log2(
                    np.max(
                        clusters_h[normal_idx] /
                        raw_h))))
        ...
        mod = mx.mod.Module(
            ...
            label_names=None)
        mod.bind(
            for_training=False, data_shapes=[
                ('data', (1, 3, 224, 224))], label_shapes=None, force_rebind=False)
        mod.set_params(
        ...
        for r in refind_bboxes:
            cv2.rectangle(raw_img, (r[0], r[1]),
                          (r[2], r[3]), (255, 255, 0), 3)
        print("Total detected faces: %d" % len(refind_bboxes))
        output_file = os.path.join(processed_folder, os.path.splitext(image_name)[
                                   0] + "_" + str(len(refind_bboxes)) + "faces" + os.path.splitext(image_name)[1])
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Run Tiny Face Detector on Images')
    # Required args
    parser.add_argument(
        "image_foldername",
        type=str,
