...
def main(test_file, test_folder, test_type, caffe_model, prototxt_file, use_gpu, threshold, nms_thresh):
    ...
    if test_type == 'facecount':
        ...
        for item in pic_to_faces:
            ...
            file = basename(item)
            _, num_detections = identify_chips(image, frame_number=-1, threshold=threshold, nms_thresh=nms_thresh,
                                               reduceby=1, net=net)
        ...
        for item in pic_to_faces:
            ...
            t = timeit.Timer(stmt='''identify_chips(image, frame_number=-1, threshold=threshold, nms_thresh=nms_thresh,
                                               reduceby=1, net=net)''', setup=setup, globals=locals())
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        type=bool,
        default=True,
        help='Use GPU, if available with nvidia-docker. Default = True.')
