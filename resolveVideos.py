...
def retrieve_reference_files(reference_faces_file, hash_table_file, current_file, current_faces):
    ...
    if cold_start:
        reference_faces, hash_table, write_to_hash = create_reference_faces(current_faces,
                                            reference_faces, hash_table, current_file)

    return reference_faces, hash_table, cold_start


# Create a reference face dictionary if no previous reference exists
def create_reference_faces(current_faces, reference_faces, hash_table, current_file):
    ...
    for current_face_id in current_faces:
        ...
        if write_to_hash:
            hash_table = add_to_hash_table(current_faces,current_face_id,hash_table)
            write_to_hash = False
    print(
...
def main(vectors_used,
    ...
    # Return only files with faces that were vectorized according to vectors_used parameter
    if vectors_used == 'dlib':
        ...
        filtered_folder = [file for file in os.listdir(
            detected_faces_folder) if file.endswith(vectors_used + "_" + 'face_detected.pickle')]

    if len(filtered_folder) == 0:
        print("No files detected for {0} vectorization technique; quitting".format(vectors_used))
        quit()
    else:
        print("{0} files detected that used {1} vectorization technique".format(len(filtered_folder),vectors_used))
    ...
    for current_file in filtered_folder:
        ...
        if not setup_completed:
            reference_faces, hash_table, cold_start = retrieve_reference_files(reference_faces_file, hash_table_file,
                                                                   current_file, current_faces)
            setup_completed = True
            # If we made the reference set from the first file, move on to next file
            if cold_start: continue
        ...
        for current_face_id in current_faces:
            ...
            match = list(np.linalg.norm(np.array([current_faces[current_face_id]['face_vec']]) -
                         np.array(reference_vectors_only), axis=1) <= tolerance)
            ...
            for index in match_indexes:
                ...
                reference_faces.update(
                    merge_face_to_reference(key, reference_faces[key], current_faces[current_face_id]))
    ...
    print("\nReviewed {0} faces and located {1} similar face vectors at threshold {2}".format(total_faces_reviewed,
                                                                                        total_duplicates, tolerance))
